...
// Commands". The actual actions depend on backup/restore (PART 21),
// self-update (PART 22), and the token/config/compliance groundwork in
// PART 11/12 — all tracked in TODO.AI.md. This file implements the full
// flag parsing, dispatch, and --help text now, and reports each action as
// honestly not yet available.
package main
...
Commands:
  backup [FILE]        Create a backup archive
...
// process exit code.
func runMaintenance(binaryName, command string) int {
	switch command {
	case "", "help", "--help", "-h":
