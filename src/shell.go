...
func bashCompletion(binaryName string) string {
	...
	b.WriteString("        --mode) COMPREPLY=($(compgen -W \"production development debug\" -- \"$cur\")); return ;;\n")
...
func zshCompletion(binaryName string) string {
	...
	b.WriteString("        --mode) _values 'mode' production development debug; return ;;\n")
...
func fishCompletion(binaryName string) string {
	...
	fmt.Fprintf(&b, "complete -c %s -n '__fish_seen_argument -l mode' -a 'production development debug'\n", binaryName)
